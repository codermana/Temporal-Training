"""Run a long-lived Worker and start one waiting approval-demo Workflow.

    python worker.py        # needs a Temporal dev server on 127.0.0.1:7233

Mirrors ApprovalWorker.java: the Workflow blocks in wait_condition until an
approve/reject Signal arrives, so you can drive it from the CLI in another shell:
...
async def main() -> None:
    ...
    async with Worker(client, task_queue=TASK_QUEUE, workflows=[ApprovalWorkflow]):
        ...
        # Stay alive so the Worker keeps polling and the Workflow stays queryable.
